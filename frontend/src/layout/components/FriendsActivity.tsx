...
const FriendsActivity = () => {
  const { isLoading, users, fetchUsers, error, onlineUsers, userActivities } = useChatStore();
  ...
  useEffect(() => {
    ...
    fetchWithToken();
  }, [fetchUsers, user, isUserLoaded, isAuthLoaded, getToken]);
        ...
        <ScrollArea className='flex-1'>
          <div className='p-4 space-y-4'>
            {users.map((user) => {
              ...
              return (
                <div key={user._id} className='cursor-pointer hover:bg-zinc-800/50 p-3 rounded-md transition-colors'>
